...
# uhvatiti one levele za svaki atribut koji imaju a. najviše buyera ili b. najveći omjer buyer/non-buyer
# za svaki taj level spremiti sve respondere koji su buyeri
# idealno bi bilo onda naći presjek svih tih skupova, da ne bude prazan ili premali i onda bi to bio skup koji bi
# imalo smisla ciljati za kupovinu
# ili napraviti logističku regresiju svih levela da se predvidi buyer i onda vidjeti koji leveli najviše pozitivno
# doprinose da bude buyer

for (cn in cns) {
  print(cn)
  print(table(rank_1$buyer, survey_data[[cn]]))
  readline(prompt="Press [enter] to continue")
}

buyer.glm <- glm(as.formula(paste0("buyer ~ ", paste(setdiff(cns, "weights"), collapse = " + "))), 
                 data = cbind(buyer = as.numeric(factor(rank_1$buyer, levels = c("non-buyer", "buyer"))) - 1, survey_data[, cns]), 
                 family="binomial")
...
sumtable.buyer.glm$odds_effect = round(exp(sumtable.buyer.glm$Estimate), 3)
sumtable.buyer.glm$explanation = "" # create a column
odds_effect_minus_1 = sumtable.buyer.glm$odds_effect - 1
sumtable.buyer.glm[odds_effect_minus_1 < 0, ]$explanation = paste0("decrease buying odds by ", round(abs(odds_effect_minus_1[odds_effect_minus_1 < 0]), 2))
sumtable.buyer.glm[odds_effect_minus_1 >= 0, ]$explanation = paste0("increase buying odds by ", round(odds_effect_minus_1[odds_effect_minus_1 >= 0], 2))
for (an in setdiff(cns, "weights")) {
...
sumtable.buyer.glm[, c("Estimate", "Pr...z..", "explanation")]
...
for (an in setdiff(cns, "weights")) {
  ...
  signif_an_rows = an_rows[an_rows$Pr...z.. < 0.05, , drop = FALSE]
  if ((nrow(signif_an_rows) > 0) && (max(signif_an_rows$Estimate) > 0)) {
    ...
    non_signif_an_rows = an_rows[an_rows$Pr...z.. >= 0.05, , drop = FALSE] # at least reference level is here
...
colnames(best_personals) = c("personal", "value", "estimate", "p-value", "explanation")
